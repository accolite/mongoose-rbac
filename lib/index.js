...
RoleSchema.methods.can = function (action, subject, scope, done) {
  mongoose.model('Role').findById(this._id, function (err, role) {
    if (err) return done(err);
    doCan.call(role, CAN_ALL, [[action, subject, scope]], done);
  });
};
    ...
    if (role.permissions) {
      actionsAndSubjects.forEach(function (as) {
        ...
        role.permissions.forEach(function (p) {
          if (p.action === as[0] && p.subject === as[1] && p.scope.indexOf(as[2]) > -1) has = true;
        });
        if (has) count++;
      });
    }
...
function plugin(schema, options) {
  ...
  schema.methods.can = function (action, subject, scope, done) {
    var obj = this;
    ...
    obj.populate('roles', function (err, obj) {
      ...
      if (obj.roles) {
        async.forEachSeries(obj.roles, function (role, next) {
          role.can(action, subject, scope, function (err, has) {
      ...
      if (obj.roles) {
        async.forEachSeries(actionsAndSubjects, function (as, nextPerm) {
          ...
          async.forEachSeries(obj.roles, function (role, nextRole) {
            role.can(as[0], as[1], as[2], function (err, has) {
    ...
    for (var i = 0; i < len; i++) {
      if (Array.isArray(rolesAndPermissions[name][i])) {
        rolesAndPermissions[name][i] = {
          ...
          scope: rolesAndPermissions[name][i][2]
        };
      }
    }
    // Create role
